...
4. **Stub built-in tools.** When the agent invokes a tool whose name is in `case.scripted_tools`, the harness returns the configured `output` (or raises with `error`). Tools NOT in `scripted_tools` either fall through to a Commission-managed MCP server (post-resolution) or surface as `tool_returned` with `isError: true` (unknown tool); the harness doesn't synthesize replies for them.
