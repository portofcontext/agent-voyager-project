...
def test_build_sets_library_conventions() -> None:
    c = commission_mod.build_commission("my-id", prompt="{input}", model="m")
    assert c.schema_version == "0.1"
    assert c.run_id == "my-id"  # run_id is the id, by convention
    assert c.prompt == "{input}" and c.model == "m"


def test_clone_carries_bulky_fields_but_resets_run_id() -> None:
    schema = {"type": "object", "properties": {"a": {"type": "string"}}}
    base = Commission(
        schema_version="0.1", run_id="old", model="m", prompt="p", output_schema=schema
    )
    ...
    assert c.model == "m"  # un-overridden base value retained


def test_empty_list_is_a_real_value_expose_none() -> None:
    c = commission_mod.build_commission("x", enabled_builtin_tools=[])
...
def test_descriptor_validation_accepts_a_known_subset() -> None:
    c = commission_mod.build_commission(
        "x", enabled_builtin_tools=["Read"], descriptor=_descriptor()
    )
...
def test_create_via_flags_writes_a_wire_commission(avp_home, monkeypatch) -> None:
    ...
    rc = cli.main(["commission", "create", "terse", "--model", "m", "--prompt", "{input}"])
    assert rc == 0
    c = library.load("terse")
    assert c.run_id == "terse" and c.model == "m" and c.prompt == "{input}"
...
def test_create_with_valid_enabled_tool_succeeds(avp_home, monkeypatch) -> None:
    monkeypatch.setattr(cli, "_describe_for_create", lambda spec: (_descriptor(), None))
    rc = cli.main(
        ["commission", "create", "x", "--agent", "demo", "--enable-tool", "Read", "--prompt", "p"]
    )
    assert rc == 0
...
def test_create_refuses_to_clobber_without_force(avp_home) -> None:
    assert cli.main(["commission", "create", "dup", "--prompt", "a"]) == 0
    assert cli.main(["commission", "create", "dup", "--prompt", "b"]) == 1  # exists
    assert cli.main(["commission", "create", "dup", "--prompt", "b", "--force"]) == 0
...
def test_create_clone_then_override(avp_home) -> None:
    schema = {"type": "object", "properties": {"a": {"type": "string"}}}
    library.save("src", Commission(schema_version="0.1", run_id="src", output_schema=schema))
    rc = cli.main(["commission", "create", "dst", "--from", "src", "--model", "m"])
    assert rc == 0
    dst = library.load("dst")
    assert dst.output_schema == schema and dst.model == "m" and dst.run_id == "dst"
