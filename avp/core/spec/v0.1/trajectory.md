...
- **`avp.agent_started`** is the **merged-state snapshot**: what the run will actually use, after the descriptor's catalog is filtered by Commission's `enabled_builtin_*` allowlists and combined with Commission's managed assets. `data` carries the settled `avp.prompt` / `avp.system_prompt` / `model` and the merged lists `avp.tools[]`, `avp.mcp_servers[]`, `avp.skills[]`, `avp.subagents[]`.
    - **`avp.tools[]` is the single authoritative bag of usable tools** the model can dispatch this run: local tools post-allowlist, plus tools from MCP servers whose dial reached `status: "connected"`. Entries that originated from an MCP server carry `avp.mcp_server_id` pointing at an entry in `avp.mcp_servers[]`; local entries omit it. Entries SHOULD carry the model-facing `description`, `inputSchema`, and `outputSchema` whenever the agent's runtime exposes them ([Agent Descriptor §2.3](./agent-descriptor.md)): the tool catalog is the dominant fixed input-token cost of every turn, and decls without it leave first-turn token usage unattributable.
...
3. The agent runs the subagent. When it terminates, the agent emits `avp.subagent_returned` with `data["avp.subagent.reason"]` (a `StopReason`) and `data["avp.subagent.result.text"]`; on the error path, `reason = error` and `result.text` carries the error string. The `data.span_id` MUST equal the matching `subagent_invoked.data.span_id`; `data.parent_span_id` MUST equal the enclosing turn span.
   ...
   Every `subagent_invoked` MUST be closed by a `subagent_returned` before `agent_stopped`. Where a runtime dispatches subagents **asynchronously** (the tool call returns a launch receipt immediately and the child reports later, out of band), the agent MUST NOT treat the receipt as the child's result, and MUST keep the frame open until the child actually reports. If the run ends while a child is still in flight, the agent MUST synthesize the close with `reason = abandoned` rather than omitting it, and `agent_stopped.data["avp.reason"]` MUST NOT be `converged`: a parent that stopped without its delegated work has produced no answer, and reporting `converged` there presents an unfinished run as a successful one. It SHOULD be `abandoned`, unless the run independently ended in `error`, `refused`, or `interrupted`, which are stronger signals and MUST be preserved.
...
5. **In-process SDK fallback.** When the agent's SDK black-boxes the child loop and never exposes the child's per-turn events (e.g. Claude Agent SDK's Task tool, which yields only `TaskNotificationMessage` with `TaskUsage` totals), the parent MAY populate `subagent_returned.data["avp.subagent.usage"]` with a `SubagentUsage` carrier (`cost_usd`, `tokens_input`, `tokens_output`, `turns`) so the supervisor sees the child's spend. Every field of the carrier is OPTIONAL, because SDKs report different subsets of it. An agent MUST omit a field it cannot measure rather than emitting `0`: per §2.1 absence is the signal for "not reported," and a zero would be indistinguishable from a measured zero to a supervisor summing spend. SDK-specific totals the carrier has no field for (Claude Agent SDK's `total_tokens` / `tool_uses` / `duration_ms`) MAY ride along as additional keys. Agents that CAN emit the child's per-turn events into the parent's trajectory (with `parent_span_id` = the invocation's `span_id`) MUST do so and MUST omit `avp.subagent.usage`; the supervisor reconstructs from raw events.
...
Skill exploration is nonetheless observable from the events that already exist: agents typically read skill bodies through ordinary file tools (Read / Grep over the skill's directory), so each load shows up as `tool_invoked` / `tool_returned` pairs whose inputs resolve under a skill's path. Agents SHOULD populate `skills[].avp.source` (the SKILL.md path / URI) so consumers can make that attribution by matching tool inputs against it; with `source` populated, per-skill usage and token cost are computable from the raw stream with no new event class.
...
| `avp.subagent_returned` | `avp://agent` | Subagent's frame closed for any reason (converged / refused / errored / interrupted / abandoned). Pairs with `subagent_invoked` by `span_id`. `avp.subagent.reason` is a `StopReason`; on `error`, `avp.subagent.result.text` carries the error string and the paired `tool_returned` sets `is_error = true`. |
...
7. Every `avp.subagent_invoked` MUST be paired with an `avp.subagent_returned` carrying the same `span_id`, emitted before `agent_stopped`. A subagent still in flight when the run ends MUST be closed with `avp.subagent.reason = abandoned`, and the run's `agent_stopped.data["avp.reason"]` MUST NOT then be `converged` (see §5.6).
8. All emitted events MUST validate against `trajectory.schema.json`.
