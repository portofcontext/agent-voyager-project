...
SPEC="$REPO/core/spec/v0.1"
RUST_OUT="$REPO/bindings/rust/src"
TS_OUT="$REPO/bindings/typescript/src"

# Conformance fixture schema lives inside the avp-conformance package (it is
# not part of the wire spec; it describes the `--built-in` harness payload).
CONFORMANCE_SCHEMA="$REPO/core/conformance/src/avp_conformance/agent-built-ins.schema.json"
