...
`avp/core/spec/v0.1/` so they cannot drift from the implementation.
...
One entry-point schema per AVP v0.1 spec, plus the unified bundle:
    ...
    avp/core/spec/v0.1/commission.schema.json        — the Commission shape
    avp/core/spec/v0.1/trajectory.schema.json        — agent-emitted Event union
    avp/core/spec/v0.1/agent-descriptor.schema.json  — AgentDescriptor shape
    avp/core/spec/v0.1/avp.schema.json               — top-level bundle (oneOf of all three)

Plus the conformance-harness fixture schema (not part of the wire spec; ships
inside the avp-conformance package so consumers can validate `--built-in` payloads):

    avp/core/conformance/src/avp_conformance/agent-built-ins.schema.json
"""
...
def main() -> int:
    ...
    # Conformance fixture schema. Lives inside the avp-conformance package (not
    # under core/spec/v0.1/) because it describes the harness `--built-in`
    # payload, not the wire format.
    conformance_dir = ROOT / "core" / "conformance" / "src" / "avp_conformance"
