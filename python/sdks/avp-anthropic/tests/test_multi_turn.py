"""Multi-turn integration tests for the AVPAgent driving AnthropicModelDriver.

These tests pin a class of bugs that single-turn driver tests cannot catch:
the agent's history shape between turns. Specifically — when the assistant
makes a tool call without text, the agent MUST still record the assistant
...
def test_tool_use_then_text_round_trip_preserves_history() -> None:
    """Full multi-turn agent exercise:
        turn 1: assistant emits a tool_use block (no text)
        agent: dispatches `read_file` locally, emits tool_returned
    ...
    Pre-fix: the agent skipped recording the assistant turn when it had no
    ...
    assert asst_msgs, (
        "turn 2 messages contain no assistant turn — the agent failed to "
    ...
    # Specific shape checks — fields live in `data` with OTel/AVP attribute names.
    assert any(
    ...
    assert any(
        isinstance(ev, TextEmittedEvent) and "EXPLANATION DONE" in ev.data.avp_text
        for ev in agent.trajectory
    )
...
def test_pure_text_turn_history_unchanged_for_string_content() -> None:
    ...
    # Force a second turn by NOT converging (use stop_reason that AVPAgent
...
def test_tool_failure_records_tool_result_in_history_so_next_turn_validates() -> None:
    """When a tool fails (ToolDriver returns ToolOutcome(error=...)), the agent
...
def test_unused_imports_silenced() -> None:
    ...
    sanity check that the AVP API surface is what these tests assume."""
    _ = (
        AgentStartedEvent,
        AgentStoppedEvent,
        ModelTurnEndedEvent,
