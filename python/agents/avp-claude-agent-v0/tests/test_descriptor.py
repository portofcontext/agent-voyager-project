"""Seam tests: avp-claude-agent's Descriptor is consistent across surfaces.

The Descriptor the agent publishes lives in three places that MUST agree:

  1. `avp_claude_agent.descriptor()` — the in-process function.
  2. `avp-claude-agent describe` — the CLI subcommand, prints JSON.
  3. `agent_described.data["avp.descriptor"]` — the on-wire event the agent
...
def test_descriptor_function_returns_valid_agent_descriptor() -> None:
    d = build_descriptor()
    assert d.agent_name == "avp-claude-agent"
    ...
    assert {t.name for t in d.built_in_tools} == set(CLAUDE_CODE_PRESET_TOOLS)
    # general-purpose is the SDK's only runtime-bundled subagent. The
    # Descriptor snapshots from the documented surface.
    assert d.built_in_subagents is not None
    assert {sa.name for sa in d.built_in_subagents} == set(CLAUDE_AGENT_SDK_BUILTIN_SUBAGENTS)
    ...
    for cap in ("skills:progressive", "thinking", "filesystem-discovery-available"):
        assert cap in d.capabilities


def test_describe_subcommand_prints_descriptor_as_json(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """`avp-claude-agent describe` MUST print the same JSON `descriptor()` returns."""
    ...
    expected = build_descriptor().model_dump(by_alias=True, exclude_none=True)
    assert parsed == expected


def test_translator_emits_run_prelude_when_descriptor_supplied() -> None:
    """When a Descriptor is passed, the translator opens the trajectory
    with run_requested → agent_described — even with a no-op SDK fake.
    The agent_described event's payload MUST equal the supplied Descriptor.
    """
    cfg = Commission.model_validate(
        {
            "schema_version": "0.1",
            "run_id": "descriptor-seam",
    ...
    on_wire = out[1].data.avp_descriptor.model_dump(by_alias=True, exclude_none=True)
    expected = descriptor.model_dump(by_alias=True, exclude_none=True)
...
def test_translator_skips_prelude_when_no_descriptor() -> None:
    """Translator constructed without a Descriptor (in-process embedded use)
    MUST NOT emit prelude. The wire only opens with run_requested when
    the agent identifies itself.
    """
    cfg = Commission.model_validate({"schema_version": "0.1", "run_id": "no-descriptor"})
    out: list[Any] = []
