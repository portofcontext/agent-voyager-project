"""Tests for native MCP support in avp-claude-agent.
...
translate AVP's `Commission.mcp_servers[]` to the SDK's shape and tag the
resulting tool events with `avp.tool.dispatch_target = "mcp_server"`.

These tests pin:
  - Commission.mcp_servers → SDK options.mcp_servers translation (stdio + http)
  ...
  - Tools whose names match `mcp__<server>__<tool>` AND server is declared
    get `dispatch_target=mcp_server` + `avp.mcp_server_id` on tool_invoked
...
# ── Translation: Commission.mcp_servers → SDK options.mcp_servers ───────────────
...
def test_http_mcp_server_with_bearer_auth_resolves_token_from_env(monkeypatch) -> None:
    ...
    translation time so the secret never lands in the wire (Commission /
...
def test_emits_mcp_server_connected_per_declared_server() -> None:
    ...
    event per Commission.mcp_servers entry — same pre-fix shape."""
...
def test_no_mcp_servers_emits_no_connected_events() -> None:
    """No Commission.mcp_servers AND no SDK-discovered servers → no events.
...
def test_mcp_prefixed_tool_call_tags_dispatch_target_and_server_id() -> None:
    ...
    assert invoked.data.avp_tool_dispatch_target == "mcp_server"
    # The avp_mcp_server_id field surfaces under its dotted alias on the wire.
    wire = invoked.model_dump(mode="json", by_alias=True, exclude_none=True)
    assert wire["data"]["avp.mcp_server_id"] == "github"
