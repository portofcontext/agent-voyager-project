...
Per-turn `model_turn_ended` events get `avp.cost.source=computed`
...
when ResultMessage arrives gets `avp.cost.source=reported` because
...
def test_per_turn_model_turn_ended_tagged_computed() -> None:
    ...
    assert ended
    for e in ended:
        assert e.data.avp_cost_source == "computed"
...
def test_reconciliation_cost_recorded_tagged_reported() -> None:
    ...
    `avp.cost.source=reported` so audit consumers see the moment we
    ...
    reported = [e for e in cost_events if e.data.avp_cost_source == "reported"]
    assert len(reported) == 1
    snap = reported[0].data.avp_state
...
def test_reconciliation_event_serializes_under_dotted_alias() -> None:
    ...
    t.run()
    reported = next(
        e for e in _by_type(out, CostRecordedEvent) if e.data.avp_cost_source == "reported"
    )
    wire = reported.model_dump(mode="json", by_alias=True, exclude_none=True)
    assert wire["data"]["avp.cost.source"] == "reported"
