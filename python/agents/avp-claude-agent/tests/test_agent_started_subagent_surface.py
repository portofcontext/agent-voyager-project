...
Three Commission shapes:
  - `commission.subagents = None` → surface SDK built-ins (`general-purpose`)
  - `commission.subagents = []` → empty surface (model has no Commission-declared
  ...
  - `commission.subagents = [Subagent(..., exposed=["*"])]` → those custom subagents only
...
`agent_started.data.skills` stays `commission.skills` only. Bundled "skills"
...
def test_subagents_empty_list_emits_empty_surface() -> None:
    """`commission.subagents = []` → null on the wire (no subagents declared).
...
def test_subagents_explicit_list_emits_only_those() -> None:
    """`commission.subagents = [Subagent(..., exposed=["*"])]` → that custom subagent only.
...
def test_skills_unset_stays_null() -> None:
    """`commission.skills = None` → null on the wire. The Claude Agent SDK does
...
def test_skills_passes_through_when_set() -> None:
    """`commission.skills` flows through with name + `avp.source` surfaced.
