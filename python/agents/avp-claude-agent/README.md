...
This means observer-pattern integrations like `avp-claude-agent` are first-class AVP-compliant — the spec doesn't ask agents to do anything an observer pattern can't do.

| AVP feature | avp-claude-agent |
|---|---|
...
Once published, the standalone install will be `pip install avp-claude-agent`.
