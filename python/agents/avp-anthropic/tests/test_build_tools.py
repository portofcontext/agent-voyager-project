...
The CLI (and any direct AVPAgent caller) goes through this helper. In the
refs-only Commission model, the helper returns the agent's built-in tool
catalog, optionally filtered by `Commission.enabled_builtin_tools`.
Managed assets (`Commission.{mcp_servers,subagents}` refs) are surfaced
via the resolver protocol (Phase 6 follow-up); this helper does not touch
them.
"""
