"""Tests for native MCP support in avp-anthropic.
...
is HTTP-only — stdio servers in `Commission.mcp_servers[]` MUST be skipped
with a warning (host them from your supervisor instead).

These tests pin:
  - Commission.mcp_servers (HTTP) → API shape `{type: url, name, url, ...}`
  - HTTP auth with `token_env` resolves to `authorization_token` at
    translation time so the secret never lands on the wire
  - Stdio servers in Commission.mcp_servers are skipped with a warning
...
from avp import Commission, McpHttpAuth, McpServer
from avp_anthropic import AnthropicModelDriver, build_anthropic_mcp_servers
...
# ── build_anthropic_mcp_servers: Commission → API shape ─────────────────────────
...
def test_end_to_end_config_mcp_servers_to_api_call(monkeypatch) -> None:
    """The CLI / supervisor pipes Commission.mcp_servers[] through
...
def test_mcp_tool_use_block_becomes_server_tool_call() -> None:
    ...
    them on `ModelResponse.server_tool_calls` so the agent can emit
