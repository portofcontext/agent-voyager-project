...
(SPEC §6) returns connection material; `AnthropicModelDriver.set_resolved_assets`
...
def test_set_resolved_assets_merges_with_preexisting_mcp_servers_param() -> None:
    """A caller that pre-populates `mcp_servers_param` (test fixture or
    legacy code) doesn't lose those entries when AVPAgent calls
