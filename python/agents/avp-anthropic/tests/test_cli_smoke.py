...
def test_cli_subagent_invocation_emits_full_lifecycle(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Crosses CLI ↔ agent ↔ subagent_driver — the seam most likely to drift,
    since the subagent driver instantiates a fresh AnthropicModelDriver from
    `avp_anthropic.subagent` (not the same import path as cli.py)."""
    ...
    # `avp_anthropic.subagent` — patch THAT import so the same mock client is
    # injected into the sub-loop too.
    from avp_anthropic import subagent as subagent_module
    ...
    nested_turns_started = [
        e
        for e in events
        if e["type"] == "avp.model_turn_started" and e["data"]["parent_span_id"] == frame_id
    ]
    ...
    started = next(e for e in events if e["type"] == "avp.agent_started")
