...
def test_environment_can_be_used_to_build_a_config() -> None:
    """Motivating use case: Commission author builds `enabled_builtin_tools`
    from discovered names, doesn't have to remember strings."""
    ...
    cfg = Commission(
        ...
        prompt="hi",
        enabled_builtin_tools=list(env.shell_tools),
    )
    assert cfg.enabled_builtin_tools == ["bash", "read_file", "write_file"]
