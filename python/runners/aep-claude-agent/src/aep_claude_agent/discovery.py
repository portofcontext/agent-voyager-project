"""Environment discovery for Config authoring.

A Config author building `cfg.allowed_tools`, `cfg.subagents`, and
`cfg.skills` needs visibility into what's actually available before
writing the Config. Some surfaces are knowable programmatically (SDK
...
splat directly into Config:

    from aep_claude_agent.discovery import discover_environment
    from aep import Config, Skill
    ...
    cfg = Config(
        ...
        skills=[
            Skill.model_validate({"name": s.name, "aep.source": str(s.source_path)})
            for s in env.filesystem_skills
        ],
    )

What this module CAN'T discover at **Config-author time** (before a
run starts):

  - **MCP-server tool lists.** Tools exposed by an MCP server (e.g.
    `mcp__github__create_issue`) are only knowable AFTER the runner
    ...
    **agent-start time** — the translator captures it then via
    ...
    (`aep.mcp.tools[]`). For Config authoring before a run, you'd need
...
class SkillInfo:
    ...
    `Skill.aep_source` when building Config.
    """
...
@dataclass(frozen=True)
class Environment:
    """Snapshot of what's available at Config-author time.
