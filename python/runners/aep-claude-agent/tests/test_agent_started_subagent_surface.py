...
Three Config shapes:
  ...
  - `cfg.subagents = []` → empty surface (model has no Config-declared
...
def test_subagents_unset_surfaces_sdk_builtin() -> None:
    """Common case: worker doesn't declare subagents in Config. The SDK
...
def test_subagents_empty_list_emits_empty_surface() -> None:
    ...
    general-purpose via the Agent tool) is acceptable: AEP Config is
...
def test_skills_passes_through_when_set() -> None:
    """`cfg.skills` flows through unchanged. The translator surfaces
    only the names (per AgentStartedData schema: `skills: list[str] | None`)."""
    from aep import Skill
    ...
    cfg = Config(
        ...
        prompt="hi",
        skills=[
            Skill.model_validate({"name": "style-guide", "aep.source": "./skills/style-guide"}),
        ],
    )
    t, out = _new_translator(cfg)
    t._emit_agent_started()
    started = _agent_started(out)
    assert started.data.skills == ["style-guide"]
