...
Lets a user write tool registration ONCE against `aep.runner.LocalTools`
and run the same setup against either runner. With aep-claude-agent the
...
Tests verify:
  ...
  - The wire shape from a LocalTools-bridge tool call: `mcp__local__<tool>`
    invoked → `aep.tool.dispatch_target=local` (bridged server is NOT
    in Config.mcp_servers, so it's a local tool from AEP's perspective)
"""
...
from aep.types import ToolInvokedEvent
from aep_claude_agent import ClaudeAgentTranslator
from aep_claude_agent.local_tools_bridge import _wrap, to_sdk_mcp_server
...
def test_translator_with_local_tools_mounts_bridged_server(monkeypatch) -> None:
    ...
    under the user-chosen key. A single AEP-LocalTools registration thus
    works against this runner without the user touching SDK APIs."""
...
def test_translator_local_tools_server_name_is_configurable(monkeypatch) -> None:
    """If a user already declared a Config.mcp_servers entry called
    ...
    cfg = Config(schema_version="0.1", run_id="lt-name", model="claude-sonnet-4-6", prompt="hi")
    out: list = []
    t = ClaudeAgentTranslator(
        cfg,
        on_event=out.append,
...
def test_bridged_tool_invocation_tags_dispatch_target_local() -> None:
    ...
    (`mcp__local__calc`). Because `local` is NOT in `Config.mcp_servers`,
    the translator's existing tag-MCP-by-Config logic correctly tags
    the resulting `tool_invoked` event with
    `aep.tool.dispatch_target=local` — same wire shape `aep-anthropic`
    ...
    # The bridged server "local" is NOT in Config.mcp_servers — so even
