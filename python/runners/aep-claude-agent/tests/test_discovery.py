"""Discovery API: helps Config authors see what's available before
...
def test_preset_tools_is_re_exported() -> None:
    """Config authors can `from aep_claude_agent import CLAUDE_CODE_PRESET_TOOLS`
    and use it directly."""
...
def test_discovers_project_subagents(tmp_path, monkeypatch) -> None:
    ...
    (agents_root / "test-runner.md").write_text("---\n---\nbody")

    env = discover_environment(cwd=tmp_path)
    names = sorted(s.name for s in env.filesystem_subagents)
    assert names == ["code-reviewer", "test-runner"]
...
def test_environment_can_be_used_to_build_a_config(tmp_path, monkeypatch) -> None:
    """The motivating use case: a Config author calls `discover_environment`,
    splats the results into Config fields, and gets a Config that
    matches what's actually available in the env."""
    ...
    cfg = Config(
        ...
        skills=[
            Skill.model_validate({"name": s.name, "aep.source": str(s.source_path.parent)})
