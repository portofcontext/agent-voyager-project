...
message in passing. These tests pin the contract:
  ...
  - Each AssistantMessage flowing through receive_response() emits the
    full per-turn AEP block (model_turn_*, text, cost) BEFORE the user
    sees the message
  ...
  - Verifier halt during message processing terminates the iterator and
    yields reason=verifier_failed
  - converged() honored when no other terminal condition fires
  - Pass-through __getattr__ forwards SDK-specific attributes
  - inject_correction queue drained via pop_corrections (parallel to
    AEPTracer.pop_corrections())
...
def _basic_config(**overrides: Any) -> Config:
    base: dict[str, Any] = {
        "schema_version": "0.1",
...
def test_receive_response_emits_per_message_aep_events_before_yield() -> None:
...
def test_pre_post_tool_use_hooks_fire_emitting_tool_lifecycle() -> None:
    ...
    tool_returned just like the runner CLI does."""
    cfg = _basic_config()
...
def test_agent_tool_with_declared_subagent_diverts_to_subagent_lifecycle() -> None:
    ...
    whose subagent_type matches a declared Config.subagent emits
    ...
    assert ret.data.aep_subagent_result_text == "found 3 things"
...
def test_at_end_verifier_runs_before_agent_stopped() -> None:
    cfg = _basic_config(
        verifiers=[
            Verifier(
                name="finalize",
...
def test_halt_verifier_during_message_iteration_yields_verifier_failed() -> None:
    """A halt verifier raised during message processing terminates the
    receive_response iterator cleanly and the run reports
    ...
    traced, out = _make_traced(cfg, messages=[msg, msg])  # two scripted turns
    ...
    asyncio.run(_run())
    # First turn fired the verifier → halt → iterator terminates.
    # Second turn never reached.
    assert iterations == 1
    stopped = _by_type(out, AgentStoppedEvent)[0]
    assert stopped.data.aep_reason == StopReason.verifier_failed
    # And a verifier_evaluated event was recorded with passed=false.
    verifiers = _by_type(out, VerifierEvaluatedEvent)
    assert any(not v.data.aep_verifier_passed for v in verifiers)


def test_inject_correction_drained_via_pop_corrections() -> None:
    """`inject_correction` queues a message; `pop_corrections()` drains
    it. The user pops after the loop body (or on the next iteration's
    start) — `after_each_turn` verifiers run AFTER the user's body
    completes, so a correction queued by the verifier for turn N is
    visible from iteration N+1's start (or after the loop ends).

    The user is responsible for handing the drained text to the SDK via
    `await client.query(correction)` — same contract as
    `AEPTracer.pop_corrections()`."""
    cfg = _basic_config(
        verifiers=[
            Verifier(
                ...
                correction_message="please fix the thing",
            )
        ]
    )
    msg = AssistantMessage(
    ...
    async def _run() -> None:
        async with traced as client:
            ...
            async for _ in client.receive_response():
                pass
            # After the loop: every queued correction is drainable here.
            drained.extend(client.pop_corrections())
            client.converged()

    asyncio.run(_run())
    assert drained == ["please fix the thing"]
...
def test_exception_inside_block_yields_reason_error() -> None:
    ...
    on the agent_stopped event (mirrors AEPRunner / translator behavior)."""
    cfg = _basic_config()
    traced, out = _make_traced(cfg, messages=[])
    ...
    with contextlib.suppress(RuntimeError):
        asyncio.run(_run())
    stopped = _by_type(out, AgentStoppedEvent)[0]
    assert stopped.data.aep_reason == StopReason.error
...
def test_options_carry_aep_hooks_so_sdk_dispatches_through_us() -> None:
...
def test_factory_pulls_config_and_on_event_from_active_tracer() -> None:
    ...
    async def _run() -> None:
        with AEPTracer(cfg, on_event=out.append):
    ...
    # AEPTracer, NOT by the translator (it's in delegated mode).
...
def test_factory_translator_shares_trace_id_with_active_tracer() -> None:
    """Delegated mode: the translator's events MUST carry the AEPTracer's
...
def test_factory_suppresses_translator_lifecycle_emission() -> None:
    ...
    in delegated mode — the outer AEPTracer already did. Two of either
    ...
    async def _run() -> None:
        with AEPTracer(_basic_config(), on_event=out.append):
            client = traced_claude_sdk_client(
...
def test_self_contained_form_still_works_unchanged() -> None:
    ...
    # (no outer AEPTracer). One of each.
    assert len(_by_type(out, AgentStartedEvent)) == 1
    assert len(_by_type(out, AgentStoppedEvent)) == 1
...
def test_factory_pushes_per_turn_spend_into_parent_tracer_state() -> None:
    ...
    AEPTracer's cumulative state — otherwise `agent_stopped.aep_state`
    reports zeros even though per-turn cost_recorded events show real
    spend, and parent-side boundary checks miss CASDK turns entirely.
