...
diverts that pair into AEP's `subagent_invoked` / `subagent_returned`
lifecycle so consumers see one consistent wire shape across runners
(driver-pattern aep-anthropic produces the rich nested-tree variant; this
runner produces the thin invoked+returned variant — same events, just
without internals).

These tests pin:
  - Config.subagents → ClaudeAgentOptions.agents translation
  ...
  - The `Agent` tool with an UNDECLARED subagent_type falls through as a
    regular tool (we don't accidentally swallow non-AEP Agent calls)
  - The `subagent_type` field is stripped from `aep.subagent.input` so
...
def _cfg_with_subagent(**overrides: Any) -> Config:
    base: dict[str, Any] = {
        "schema_version": "0.1",
        "run_id": "t-sa",
...
def test_config_subagents_translate_to_sdk_agents_dict() -> None:
    """Config.subagents must populate ClaudeAgentOptions.agents with the
    ...
    assert payload["prompt"] == "You are a researcher."  # AEP system_prompt → SDK prompt
    assert payload["model"] == "claude-haiku-4-5-20251001"
    assert payload["tools"] == ["bash"]


def test_no_subagents_yields_no_agents_kwarg() -> None:
    """Backwards-compat: a Config without subagents MUST NOT populate
    options.agents (so existing setups that assume the SDK uses its
    filesystem-loaded agents continue to work)."""
    cfg = Config(schema_version="0.1", run_id="t", model="claude-sonnet-4-6")
    t, _ = _make_translator(cfg)
    opts = t._build_sdk_options()
...
def test_pre_tool_use_on_agent_with_declared_subagent_emits_subagent_invoked() -> None:
    ...
    # AEP wire records what the parent agent meant to pass.
    assert ev.data.aep_subagent_input == {"prompt": "find auth handlers"}
...
def test_post_tool_use_for_subagent_emits_subagent_returned() -> None:
    ...
    assert returned[0].data.aep_subagent_result_text == "found 3 handlers in src/auth/"
...
def test_subagent_invoked_and_returned_share_frame_span_id() -> None:
    ...
    their results, and AEP's wire shape stops being a tree."""
    cfg = _cfg_with_subagent()
    t, out = _make_translator(cfg)
...
def test_agent_tool_with_undeclared_subagent_falls_through_as_tool() -> None:
    """Safety: only a tool_use whose `subagent_type` matches a declared AEP
...
def test_subagent_helper_handles_multiple_invocations_in_sequence() -> None:
    ...
    assert invoked[0].data.aep_subagent_invocation_id != invoked[1].data.aep_subagent_invocation_id
