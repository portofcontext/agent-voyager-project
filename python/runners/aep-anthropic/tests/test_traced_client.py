...
The wrapper hides AEPTracer behind a context manager so user code looks
...
These tests pin the contract:
  ...
  - Boundary: when crossed, the next `messages.create()` raises
    `BoundaryExhausted`; the wrapper's `__exit__` swallows that exception
  - `client.tool(...)` and `client.subagent(...)` delegate to the
    internal tracer
  - Pass-through for non-wrapped attributes via `__getattr__`
  - `should_stop()` available for poll-style loops that prefer it
"""
...
def test_create_call_emits_one_turn_pair() -> None:
    ...
    assert len(text_events) == 1 and text_events[0].data.aep_text == "hello"
...
def test_token_extraction_matches_anthropic_model_driver_convention() -> None:
    """AEP convention: tokens_input INCLUDES cache reads. SDK reports
    ...
    assert ended.data.aep_cost_usd > 0
...
def test_cost_zero_for_unknown_model() -> None:
    ...
    with warnings.catch_warnings():
        ...
        with AnthropicTracedClient(fake, config=_basic_config(), on_event=out.append) as client:
            ...
            client.converged()
    ended = _by_type(out, ModelTurnEndedEvent)[0]
    assert ended.data.aep_cost_usd == 0.0
...
def test_boundary_exhausted_raised_on_next_create_after_max_steps() -> None:
    """`max_steps: 1` means the SECOND `create()` MUST raise
    BoundaryExhausted before the SDK is called. The wrapper's `__exit__`
    swallows it; user code outside the `with` doesn't see it."""
    out: list = []
    fake = _FakeAnthropic([_resp(text="first"), _resp(text="second")])
    cfg = _basic_config(boundary=Boundary(max_steps=1))

    with AnthropicTracedClient(fake, config=cfg, on_event=out.append) as client:
        client.messages.create(model="claude-sonnet-4-6", messages=[])
        with pytest.raises(BoundaryExhausted):
            client.messages.create(model="claude-sonnet-4-6", messages=[])
    # Second call never reached the SDK.
    assert len(fake.messages.calls) == 1
    # agent_stopped reflects the boundary that fired.
    stopped = _by_type(out, AgentStoppedEvent)[0]
    assert stopped.data.aep_reason == StopReason.turn_limit


def test_boundary_exhausted_propagates_out_of_loop_and_swallowed_by_with() -> None:
    """The intended loop pattern: `while True: client.messages.create(...)`.
    When boundary fires, the exception escapes the loop AND escapes the
    `with` — `__exit__` returns True so the caller sees a clean exit."""
    out: list = []
    fake = _FakeAnthropic([_resp(text="t1"), _resp(text="t2"), _resp(text="t3")])
    ...
    with AnthropicTracedClient(fake, config=cfg, on_event=out.append) as client:
        while True:
            client.messages.create(model="claude-sonnet-4-6", messages=[])
            iterations += 1
    # Two successful turns, then the third create() raises and the with
    # block exits. Iterations == 2.
    assert iterations == 2
    stopped = _by_type(out, AgentStoppedEvent)[0]
    assert stopped.data.aep_reason == StopReason.turn_limit


def test_should_stop_available_for_poll_style_loops() -> None:
    """Polling is still supported for callers who prefer it. After the
    second turn, `should_stop()` returns True — same boundary semantics
    as the exception path, just non-raising."""
    out: list = []
    fake = _FakeAnthropic([_resp(text="t1"), _resp(text="t2"), _resp(text="t3")])
    cfg = _basic_config(boundary=Boundary(max_steps=2))
...
def test_subagent_context_manager_emits_subagent_invoked_and_returned() -> None:
    ...
    assert inv.data.span_id == ret.data.span_id
    assert ret.data.aep_subagent_result_text == "found 1"
...
def test_reused_client_raises() -> None:
    """A new wrapper per run — same Config can't be reused on a single
    instance."""
    fake = _FakeAnthropic([_resp(text="x")])
    client = AnthropicTracedClient(fake, config=_basic_config(), on_event=lambda _: None)
    with client:
...
def test_beta_messages_create_is_instrumented() -> None:
    """`client.beta.messages.create()` MUST emit the same AEP events as
    ...
    with AnthropicTracedClient(fake, config=_basic_config(), on_event=out.append) as client:
        ...
        client.converged()
    assert len(_by_type(out, ModelTurnEndedEvent)) == 1
    assert _by_type(out, TextEmittedEvent)[0].data.aep_text == "from beta"
...
def test_wrap_anthropic_returns_proxy_that_emits_to_active_tracer() -> None:
    ...
    from aep_anthropic import wrap_anthropic
...
def test_wrap_anthropic_raises_outside_active_tracer() -> None:
    ...
    `with AEPTracer(...)` block fails loud, not silent — better than
    dropping events the user expected to see."""
    from aep_anthropic import wrap_anthropic

    fake = _FakeAnthropic([_resp(text="x")])
    client = wrap_anthropic(fake)
    with pytest.raises(RuntimeError, match="No active AEPTracer"):
        client.messages.create(model="claude-sonnet-4-6", messages=[])


def test_wrap_anthropic_works_across_multiple_traces() -> None:
    """The big payoff of the wrap-once-use-many pattern: the same
    wrapped client survives multiple `with AEPTracer` blocks. Compare
    ...
    with AEPTracer(cfg_a, on_event=out_a.append):
        client.messages.create(model="claude-sonnet-4-6", messages=[])

    out_b: list = []
    cfg_b = _basic_config(run_id="run-b")
    with AEPTracer(cfg_b, on_event=out_b.append):
        client.messages.create(model="claude-sonnet-4-6", messages=[])
...
def test_module_level_tool_helper_works_with_wrap_anthropic() -> None:
    ...
    with AEPTracer(_basic_config(), on_event=out.append):
        client.messages.create(model="claude-sonnet-4-6", messages=[])
        with aep_tool(call_id="c1", name="bash", input={"command": "ls"}) as t:
            t.record("ok")

    assert _by_type(out, ToolInvokedEvent)[0].data.gen_ai_tool_name == "bash"
    assert _by_type(out, ToolReturnedEvent)[0].data.aep_tool_result_text == "ok"
...
def test_wrap_anthropic_handles_async_client() -> None:
    ...
    assert _by_type(out, TextEmittedEvent)[0].data.aep_text == "async hello"
