"""Discovery API for Config authoring (Anthropic runner side).
...
surface is just the runner's shell built-ins and the API-hosted tool
kinds the response parser knows about.
"""

from __future__ import annotations

from aep_anthropic import (
...
def test_shell_tool_names_re_exported() -> None:
    """`from aep_anthropic import SHELL_TOOL_NAMES` works for Config
...
def test_discover_environment_returns_constants() -> None:
    ...
    can write one discovery helper that works across runners."""
...
def test_environment_can_be_used_to_build_a_config() -> None:
    """Motivating use case: Config author builds allowed_tools from
