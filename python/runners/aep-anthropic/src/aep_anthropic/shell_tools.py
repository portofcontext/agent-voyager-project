...
The runner is supposed to ship its own built-ins (per SPEC.md §8 the runner's
local-impl tools are runner-specific and not declared in Config.tools). This
module is what aep-anthropic's CLI registers by default. Names match what the
...
SHELL_TOOL_SCHEMAS: list[dict[str, Any]] = [
    {
        "name": "bash",
        "description": (
            "Run a shell command in the runner's working directory. Returns combined "
