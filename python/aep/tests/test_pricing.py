...
The price table lives in `aep/data/prices.json` and is loaded by
`aep.load_default_prices()`. Both runner packages import from
`aep.compute_cost`, so a single table change covers them. Each
`model_turn_ended` event carries `aep.cost.source` tagging the
provenance of `aep.cost_usd` — `computed`, `reported`, or `unknown`.
"""
...
from aep.types import ModelTurnEndedEvent
...
def test_compute_cost_uses_per_million_pricing() -> None:
    ...
    cost, source = compute_cost(
        "m1",
        input_tokens=1_000_000,  # AEP convention: includes cache reads/writes
...
# ── Wire shape: aep.cost.source on model_turn_ended ────────────────────────


def test_model_turn_ended_emits_computed_when_driver_default() -> None:
    """The reference runner stamps `aep.cost.source` from
    `ModelResponse.cost_source`, defaulting to `computed`."""
    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="cost-default", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=10,
    ...
    runner.run()
    ended = next(e for e in runner.trajectory if isinstance(e, ModelTurnEndedEvent))
    assert ended.data.aep_cost_source == "computed"
...
def test_model_turn_ended_emits_reported_when_driver_says_so() -> None:
    ...
    `cost_source="reported"` and the runner forwards that
    verbatim — no recomputation, no override."""
    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="cost-prov", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=10,
    ...
    runner.run()
    ended = next(e for e in runner.trajectory if isinstance(e, ModelTurnEndedEvent))
    assert ended.data.aep_cost_source == "reported"
    assert ended.data.aep_cost_usd == 0.99


def test_model_turn_ended_serializes_cost_source_under_dotted_alias() -> None:
    """Wire spelling is `aep.cost.source` (dotted), matching the rest
    of the AEP attribute namespace. Pydantic field is `aep_cost_source`."""
    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="cost-wire", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=1,
    ...
    assert wire["data"]["aep.cost.source"] == "computed"
    # Sanity: the pydantic field name does NOT leak into the wire dump.
    assert "aep_cost_source" not in wire["data"]
