...
(`aep.total_tokens`, `aep.total_cost_usd`, `aep.total_turns`,
`aep.duration_ms`) MUST equal the matching fields inside `aep.state`
when populated. Pydantic's model_validator enforces this at
construction time so drift is caught at the runner, not at the
supervisor.
...
from aep.types import AgentStoppedData, RunStateSnapshot, StopReason
...
def test_matching_top_level_and_snapshot_construct_cleanly() -> None:
    """The runner stamps both with the same values — happy path."""
    AgentStoppedData(
        **_span_kwargs(),
        **{
            "aep.reason": StopReason.converged,
            "aep.state": _snap(),
            "aep.total_cost_usd": 0.5,
            "aep.total_tokens": 1000,
            "aep.total_turns": 3,
            "aep.duration_ms": 4500,
        },
    )


def test_top_level_none_with_snapshot_present_is_allowed() -> None:
    """The convenience fields are optional; a runner that wants to skip
...
def test_disagreeing_total_cost_raises_at_construction() -> None:
    with pytest.raises(ValueError, match=r"aep\.total_cost_usd"):
        AgentStoppedData(
            **_span_kwargs(),
            **{
                "aep.reason": StopReason.converged,
                "aep.state": _snap(total_cost_usd=0.5),
                "aep.total_cost_usd": 0.42,  # <-- disagrees with snapshot
            },
        )


def test_disagreeing_total_tokens_raises_at_construction() -> None:
    with pytest.raises(ValueError, match=r"aep\.total_tokens"):
        AgentStoppedData(
            **_span_kwargs(),
            **{
                "aep.reason": StopReason.converged,
                "aep.state": _snap(total_tokens=1000),
                "aep.total_tokens": 999,
            },
        )


def test_disagreeing_total_turns_raises_at_construction() -> None:
    with pytest.raises(ValueError, match=r"aep\.total_turns"):
        AgentStoppedData(
            **_span_kwargs(),
            **{
                "aep.reason": StopReason.converged,
                "aep.state": _snap(total_turns=3),
                "aep.total_turns": 4,
            },
        )
...
def test_disagreeing_duration_ms_raises_at_construction() -> None:
    with pytest.raises(ValueError, match=r"aep\.duration_ms"):
        AgentStoppedData(
            **_span_kwargs(),
            **{
                "aep.reason": StopReason.converged,
                "aep.state": _snap(duration_ms=4500),
                "aep.duration_ms": 4501,
            },
        )
...
def test_snapshot_optional_field_none_means_equivalence_skipped() -> None:
    ...
    AgentStoppedData(
        ...
        **{
            "aep.reason": StopReason.converged,
            "aep.state": _snap(duration_ms=None),
            "aep.duration_ms": 4500,  # snapshot side is None — no comparison
        },
    )
