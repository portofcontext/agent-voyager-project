"""Tests for AEPTracer — the drop-in alternative to AEPRunner.

These tests pin the contract that matters: events emitted by AEPTracer MUST
be byte-equivalent to events AEPRunner produces for the same Config and
the same set of (turn, tool, subagent) operations. Consumers downstream
of the wire can't tell whether the trajectory came from a runner or a
...
Coverage:
  ...
  - Boundary: max_steps / max_cost_usd / max_tokens via should_stop()
  ...
  - Verifier dispatch at all four trigger points
  - inject_correction queues a message; pop_corrections drains it
...
def test_minimal_run_emits_full_lifecycle() -> None:
    ...
    MUST emit the same skeleton AEPRunner does for an empty-script run."""
    out: list = []
    cfg = _basic_config()
    with AEPTracer(cfg, on_event=out.append) as tracer:
...
def test_emitted_events_match_aeprunner_envelope_shape() -> None:
    """CloudEvents 1.0 envelope invariants. Same as AEPRunner."""
    out: list = []
    with AEPTracer(_basic_config(), on_event=out.append) as tracer:
        with tracer.turn() as turn:
...
def test_should_stop_fires_when_max_steps_would_be_exceeded() -> None:
    """max_steps: 2 → after 2 turns, should_stop() returns True. Strict
    >, projected count rule (matches AEPRunner SPEC §9.2)."""
    out: list = []
    cfg = _basic_config(boundary=Boundary(max_steps=2))
    with AEPTracer(cfg, on_event=out.append) as tracer:
        turns = 0
        while not tracer.should_stop():
            with tracer.turn() as turn:
                turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0, text=f"turn {turns + 1}")
            turns += 1
    assert turns == 2, f"max_steps=2 MUST yield exactly 2 turns, got {turns}"
    assert tracer.state.total_turns == 2


def test_should_stop_fires_when_max_cost_exceeded() -> None:
    """Strict greater-than. 0.05 of 0.10 budget allows another turn; 0.11
    over 0.10 stops."""
    out: list = []
    cfg = _basic_config(boundary=Boundary(max_cost_usd=0.10))
    with AEPTracer(cfg, on_event=out.append) as tracer:
        turns = 0
        while not tracer.should_stop():
            with tracer.turn() as turn:
                turn.record(tokens_input=1, tokens_output=1, cost_usd=0.06, text="t")
            turns += 1
    # First turn brings cost to 0.06 (< 0.10 → ok). Second turn brings it to
    # 0.12 (> 0.10 → stop). Third should not fire.
    assert turns == 2
...
def test_tool_call_records_invoked_and_returned() -> None:
    ...
    assert ret.data.aep_tool_result_text == "file1\nfile2\n"
...
def test_tool_call_reject_marks_rejected_on_returned() -> None:
    ...
    ret = _by_type(out, ToolReturnedEvent)[0]
    assert ret.data.aep_tool_rejected is True
    assert ret.data.aep_tool_rejection_reason == "dangerous_path"


# ── Subagent ─────────────────────────────────────────────────────────────────


def _cfg_with_subagent() -> Config:
...
def test_subagent_scope_emits_invoked_and_returned_with_paired_span() -> None:
    out: list = []
    with AEPTracer(_cfg_with_subagent(), on_event=out.append) as tracer:
        with tracer.turn() as turn:
    ...
    assert ret.data.aep_subagent_result_text == "bullet"
...
def test_subagent_usage_rolls_into_parent_state_and_appears_on_returned() -> None:
    ...
    preserved on subagent_returned.aep.subagent.usage so consumers can
    ...
    stopped = _by_type(out, AgentStoppedEvent)[0]
    parent_state = stopped.data.aep_state
...
def test_subagent_fail_emits_subagent_failed_not_returned() -> None:
    ...
    assert fail.data.aep_subagent_error == "subagent rejected the task"
    assert fail.data.aep_subagent_error_code == "rejected"


def test_subagent_undeclared_name_raises() -> None:
    """Calling tracer.subagent(name=X) where X isn't in Config.subagents
    is a programming error — fail loudly, not silently."""
    import pytest

    with AEPTracer(_basic_config(), on_event=lambda _: None) as tracer:
...
def test_verifier_after_each_turn_fires_after_each_turn() -> None:
    out: list = []
    cfg = _basic_config(
        verifiers=[
            Verifier(
    ...
    with AEPTracer(cfg, on_event=out.append) as tracer:
        with tracer.turn() as turn:
            turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0, text="t1")
        with tracer.turn() as turn:
            turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0, text="t2")
        tracer.converged()
    verifiers = _by_type(out, VerifierEvaluatedEvent)
    assert len(verifiers) == 2, "after_each_turn fires after every turn"
    assert all(v.data.aep_verifier_passed for v in verifiers)


def test_verifier_at_end_fires_before_agent_stopped() -> None:
    out: list = []
    cfg = _basic_config(
        verifiers=[
            Verifier(
    ...
    types = _types(out)
    # at_end verifier fires before agent_stopped
    assert types.index("VerifierEvaluatedEvent") < types.index("AgentStoppedEvent")


def test_verifier_on_tool_fires_after_tool_returned() -> None:
    out: list = []
    cfg = _basic_config(
        verifiers=[
            Verifier(
    ...
    with AEPTracer(cfg, on_event=out.append) as tracer:
        with tracer.turn() as turn:
            turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0)
        with tracer.tool(call_id="c1", name="bash", input={}) as t:
            t.record("ok")
        tracer.converged()
    types = _types(out)
    assert "VerifierEvaluatedEvent" in types
    assert types.index("ToolReturnedEvent") < types.index("VerifierEvaluatedEvent")


def test_verifier_inject_correction_queues_message() -> None:
    out: list = []
    cfg = _basic_config(
        verifiers=[
            Verifier(
                ...
                correction_message="please retry, you missed something",
            )
        ]
    )
    with AEPTracer(cfg, on_event=out.append) as tracer:
        with tracer.turn() as turn:
            turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0)
        corrections = tracer.pop_corrections()
        assert corrections == ["please retry, you missed something"]
        # Second pop drains the queue
        assert tracer.pop_corrections() == []
        tracer.converged()


def test_verifier_halt_marks_should_stop_and_stops_with_verifier_failed() -> None:
    out: list = []
    cfg = _basic_config(
        verifiers=[
            Verifier(
    ...
    with AEPTracer(cfg, on_event=out.append) as tracer:
        ...
        while not tracer.should_stop():
            with tracer.turn() as turn:
                turn.record(tokens_input=1, tokens_output=1, cost_usd=0.0)
            turns += 1
    # First turn ran, verifier halted, loop exits before turn 2.
    assert turns == 1
    stopped = _by_type(out, AgentStoppedEvent)[0]
    assert stopped.data.aep_reason == StopReason.verifier_failed


def test_verifier_before_first_turn_fires_once_before_first_turn() -> None:
    """Same trigger semantics as AEPRunner — fires exactly once, on the
    first call to tracer.turn(), before model_turn_started."""
    out: list = []
        ...
        cfg = _basic_config(
            verifiers=[
                Verifier(
                    ...
                    source=VerifierSourceShell(shell=f"touch {marker}.first"),
                )
            ]
        )
        with AEPTracer(cfg, on_event=out.append) as tracer:
            ...
            tracer.converged()
        types = _types(out)
        # Verifier fires before the first model_turn_started.
        v_idx = types.index("VerifierEvaluatedEvent")
        m_idx = types.index("ModelTurnStartedEvent")
        assert v_idx < m_idx
        # Only one verifier dispatch (not two — fires once at the boundary).
        assert types.count("VerifierEvaluatedEvent") == 1
    finally:
        for suffix in ("", ".first"):
            try:
                os.unlink(marker + suffix)
...
def test_format_event_covers_lifecycle_events() -> None:
    """`format_event` MUST handle every event type AEPTracer emits without
...
def test_print_event_writes_to_stdout(capsys) -> None:
    ...
    from aep import print_event

    with AEPTracer(_basic_config(), on_event=print_event) as tracer:
        with tracer.turn() as turn:
