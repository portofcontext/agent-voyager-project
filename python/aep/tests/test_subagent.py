"""Tests for the Subagent Config primitive and the subagent event lifecycle.

Subagent is the AEP analogue of Claude Agent SDK `AgentDefinition`, Google
...
These tests pin:
  - Config accepts a list of Subagent objects with the documented shape
...
def test_config_accepts_subagents_with_full_environment_slice() -> None:
    """A Subagent carries an environment slice mirroring Config — its own
    ...
    cfg = Config(
        ...
        subagents=[
            Subagent(
                ...
                skills=[Skill(name="search-tips", **{"aep.source": "./skills/search-tips"})],
                verifiers=[
                    Verifier(
                        name="no-edits",
                        trigger="at_end",
                        source=VerifierSourceShell(shell="git diff --quiet"),
                    )
                ],
                boundary=Boundary(max_steps=5, max_cost_usd=0.10),
            )
        ],
    )
    ...
    assert sa.boundary and sa.boundary.max_steps == 5
    assert sa.tools and sa.tools[0].name == "grep"
...
def test_subagent_recursion_declared_in_type() -> None:
    """Subagents may declare their own subagents. Whether a runner enables
...
def test_subagent_invoked_event_parses_and_round_trips() -> None:
    ...
    payload = _envelope(
        ...
        "r-1",
        {
            **_span(parent=parent_turn_span, span_id=frame_span),
            ...
            "aep.subagent.invocation_id": "sa-1",
            "aep.subagent.input": {"prompt": "find auth handlers"},
        },
    )
    ev = parse_event(payload)
    assert isinstance(ev, SubagentInvokedEvent)
    assert ev.data.gen_ai_agent_name == "code-explorer"
    assert ev.data.aep_subagent_invocation_id == "sa-1"
    ...
    assert "aep.subagent.invocation_id" in wire["data"]
...
def test_subagent_returned_event_carries_usage_rollup_and_pairs_with_invoked() -> None:
    ...
    payload = _envelope(
        ...
        "r-1",
        {
            **_span(parent=parent_turn_span, span_id=frame_span),
            ...
            "aep.subagent.invocation_id": "sa-1",
            "duration_ms": 4321,
            "aep.subagent.result.text": "Found 3 handlers in src/auth/.",
            "aep.subagent.reason": "converged",
            "aep.subagent.usage": {
    ...
    assert ev.data.span_id == frame_span
    assert ev.data.aep_subagent_reason == "converged"
    assert ev.data.aep_subagent_usage.total_turns == 3
    assert ev.data.aep_subagent_usage.total_cost_usd == 0.012

    wire = event_to_wire(ev)
    assert "aep.subagent.result.text" in wire["data"]
    assert "aep.subagent.usage" in wire["data"]
    assert wire["data"]["aep.subagent.reason"] == "converged"
    ...
    payload = _envelope(
        ...
        "r-1",
        {
            **_span(),
            ...
            "aep.subagent.invocation_id": "sa-1",
            "duration_ms": 12,
            "aep.subagent.error": "subagent driver not configured",
            "aep.subagent.error.code": "not_configured",
        },
    )
    ev = parse_event(payload)
    assert isinstance(ev, SubagentFailedEvent)
    assert ev.data.aep_subagent_error == "subagent driver not configured"
    assert ev.data.aep_subagent_error_code == "not_configured"
