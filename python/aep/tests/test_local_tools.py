...
This is the AEP equivalent of Claude Agent SDK's `@tool` /
`create_sdk_mcp_server()` pattern: users register named callables at
runtime; the reference runner dispatches them like any other local tool
(no wire changes, no special-casing in `runner.py`).
...
Tests cover:
  ...
  - End-to-end: pre_tool verifier gates a LocalTools call (proves the
    boundary applies to in-process tools, not just RPC)
"""
...
from aep import Config, Verifier
from aep.runner.drivers import ModelResponse, ScriptedToolCall, ToolOutcome
...
def test_fallback_driver_handles_unregistered_tools() -> None:
    ...
    callables over runner built-ins like ShellTools."""
...
def test_schemas_property_combines_local_and_fallback_schemas() -> None:
    ...
    then fallback entries we don't shadow. Lets the runner declare
...
def test_runner_dispatches_local_tool_and_emits_full_lifecycle() -> None:
    ...
    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="lt-e2e", model="test/mock"),
    ...
    invoked = _by_type(runner.trajectory, ToolInvokedEvent)
    returned = _by_type(runner.trajectory, ToolReturnedEvent)
    assert len(invoked) == 1
    assert len(returned) == 1
    assert returned[0].data.aep_tool_result_text == "hi 1"
    # No tool_failed.
...
def test_runner_routes_local_tool_with_fallback_to_correct_handler() -> None:
    ...
    the runner doesn't know which side dispatched."""
    fallback = ScriptedTools({"shellish": {"output": "shell-output"}})
    tools = LocalTools(fallback=fallback)
    tools.register("calc", lambda inp: {"r": 42}, description="x", input_schema={})

    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="lt-mixed", model="test/mock"),
        model=_model_calling("calc"),
        tools=tools,
        supervisor=ScriptedSupervisor(),
    )
    runner.run()
    returned = _by_type(runner.trajectory, ToolReturnedEvent)[0]
    # The dict return was JSON-coerced for the result text.
    assert '"r": 42' in returned.data.aep_tool_result_text
...
def test_local_tool_exception_emits_tool_failed_not_tool_returned() -> None:
    ...
    runner = AEPRunner(
        config=Config(schema_version="0.1", run_id="lt-fail", model="test/mock"),
    ...
    assert len(failed) == 1
    assert "ValueError" in failed[0].data.aep_tool_error
    assert not _by_type(runner.trajectory, ToolReturnedEvent)


def test_pre_tool_verifier_gates_local_tool_dispatch() -> None:
    """Critical seam: verifier triggers fire BEFORE the runner asks
    `is_local`, so a `pre_tool:<name>` verifier with on_failure=halt
    blocks a LocalTools callable just like it would block an RPC
    tool. In-process tools don't bypass the boundary."""
    cfg = Config(
        schema_version="0.1",
        run_id="lt-gate",
        model="test/mock",
        verifiers=[
            Verifier(
                ...
                # A shell verifier that always fails — gates the call.
                source={"shell": "false"},
                on_failure="halt",
            )
        ],
    ...
    tools.register(
        "deploy",
        lambda inp: ToolOutcome(output="DEPLOYED"),
        description="x",
        input_schema={},
    )
    ...
    runner = AEPRunner(
        config=cfg,
        model=_model_calling("deploy"),
        tools=tools,
        supervisor=ScriptedSupervisor(),
    )
    stopped = runner.run()

    # Verifier fired and failed.
    verifiers = _by_type(runner.trajectory, VerifierEvaluatedEvent)
    assert verifiers and not verifiers[0].data.aep_verifier_passed
    # Run halted with verifier_failed — the LOCAL tool never ran.
