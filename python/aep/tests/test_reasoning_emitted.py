...
The wire carries `aep.reasoning.text`, optional `aep.reasoning.signature`,
and `aep.reasoning.redacted` for encrypted-only blocks.
"""

from __future__ import annotations

from aep import Config
from aep.runner.drivers import ModelResponse, ReasoningBlock
...
def _runner(model: ScriptedModel) -> AEPRunner:
    cfg = Config(schema_version="0.1", run_id="reasoning", model="test/mock")
    return AEPRunner(
        config=cfg,
        model=model,
        tools=ScriptedTools(),
...
def test_plain_thinking_block_emits_reasoning_event() -> None:
    ...
    reasoning = _by_type(runner.trajectory, ReasoningEmittedEvent)
    assert len(reasoning) == 1
    r = reasoning[0]
    assert r.data.aep_reasoning_text == "let me think step by step..."
    assert r.data.aep_reasoning_signature == "sig-abc"
    ...
    assert "aep.reasoning.redacted" not in wire["data"]
...
def test_redacted_thinking_records_occurrence_with_no_text() -> None:
    ...
    runner.run()
    r = _by_type(runner.trajectory, ReasoningEmittedEvent)[0]
    assert r.data.aep_reasoning_text == ""
    assert r.data.aep_reasoning_redacted is True
    assert r.data.aep_reasoning_signature == "enc-blob"
...
def test_reasoning_emitted_before_text_emitted() -> None:
    ...
    when the runner emits both within one turn."""
    runner = _runner(
        ScriptedModel(
            [
                ModelResponse(
...
def test_no_reasoning_blocks_emits_no_reasoning_events() -> None:
    ...
    runner.run()
    assert not _by_type(runner.trajectory, ReasoningEmittedEvent)
    assert _by_type(runner.trajectory, ModelTurnEndedEvent)
    assert _by_type(runner.trajectory, TextEmittedEvent)
...
def test_multiple_reasoning_blocks_each_emit_their_own_event() -> None:
    ...
    runner.run()
    reasoning = _by_type(runner.trajectory, ReasoningEmittedEvent)
    assert [r.data.aep_reasoning_text for r in reasoning] == [
        "first thought",
        "second thought",
    ]
...
def test_reasoning_event_serializes_under_dotted_aliases() -> None:
    ...
    r = _by_type(runner.trajectory, ReasoningEmittedEvent)[0]
    wire = r.model_dump(mode="json", by_alias=True, exclude_none=True)
    assert wire["type"] == "aep.reasoning_emitted"
    assert wire["data"]["aep.reasoning.text"] == "t"
    assert wire["data"]["aep.reasoning.signature"] == "s"
    assert wire["data"]["aep.reasoning.redacted"] is True
