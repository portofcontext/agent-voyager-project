...
tool_returned events on the AEP wire — not get rolled up into the
model_turn_ended cost.

The driver populates `ModelResponse.server_tool_calls` with a
`ServerToolCall` per inline call; the runner emits synthetic
...
per-call detail for server-side and runner-dispatched tools.
"""
...
def _runner(model: ScriptedModel) -> AEPRunner:
    cfg = Config(schema_version="0.1", run_id="server-tool", model="test/mock")
    return AEPRunner(
        config=cfg,
        model=model,
        tools=ScriptedTools(),
...
def test_server_tool_call_emits_invoked_and_returned() -> None:
    ...
    assert inv.data.aep_tool_dispatch_target == "mcp_server"
    ...
    assert wire["data"]["aep.mcp_server_id"] == "weather"

    ret = returned[0]
    assert ret.data.gen_ai_tool_call_id == "srvtu_01"
    assert ret.data.aep_tool_result_text == "sunny, 72F"
    assert ret.data.duration_ms == 42


def test_server_tool_call_pairs_invoked_and_returned_on_same_span() -> None:
    """Per AEP wire convention, paired events for the same tool call MUST
    ...
    invoked = _by_type(runner.trajectory, ToolInvokedEvent)[0]
...
def test_no_server_tool_calls_emits_no_synthetic_events() -> None:
    ...
    runner.run()
    assert not _by_type(runner.trajectory, ToolInvokedEvent)
    assert not _by_type(runner.trajectory, ToolReturnedEvent)
