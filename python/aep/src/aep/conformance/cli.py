"""`aep-conformance` CLI — the unified entry point for AEP wire-level verification.

Subcommands:
    aep-conformance run [--suite DIR | --case FILE]   — execute conformance cases
    aep-conformance validate [--suite DIR]            — schema-validate case files
    aep-conformance check-coverage                    — every event type has ≥1 case
...
from aep.conformance.coverage import check_coverage
from aep.conformance.harness import run_case, run_suite
from aep.conformance.validate import validate_suite
...
def _default_paths() -> tuple[Path | None, Path | None, Path | None]:
    ...
    return (
        ...
        root / "spec" / "v0.1" / "aep.schema.json",
...
def _cmd_check_coverage(args: argparse.Namespace) -> int:
    ...
    if schema is None:
        print(
            "error: could not find spec/v0.1/aep.schema.json; pass --schema explicitly",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        ...
        "or add to DEFERRED_COVERAGE in aep.conformance.coverage with a reason if no\n"
        "v0.1 runner emits it.",
        file=sys.stderr,
    )
    return 1
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="AEP v0.1 wire-level conformance: run cases, validate them, check coverage.",
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    p_run = sub.add_parser("run", help="execute conformance cases against the reference runner")
    ...
    p_cov.add_argument("--schema", type=Path, help="path to spec/v0.1/aep.schema.json")
