...
class Refusal:
    ...
    A refusal terminates the turn — the runner emits
    `aep.refusal_recorded`, then stops with `StopReason.refused` (a
...
class ReasoningBlock:
    ...
    runner doesn't need provider-specific parsing.
...
class ServerToolCall:
    ...
    requested and the runner dispatches. A `ServerToolCall` already
    ...
    this list so the runner can emit synthetic tool_invoked /
    tool_returned events for per-call wire fidelity, parented to the
    turn span. No runner dispatch happens — these events are
    informational only.
    """
    ...
    # `aep.tool.subtype` discriminator on the wire — `web_search`,
...
class ModelResponse:
    ...
    # `cost_recorded` events as `aep.cost.source` so audit consumers can
    # filter by trust.
