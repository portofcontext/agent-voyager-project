...
that run inside the SDK process. AEP's equivalent is this driver: a
runtime registry of named callables that the reference runner
dispatches via the existing `ToolDriver` protocol — no wire changes,
no special-casing in `runner.py`.

Design points:

  - Wire-pure Config. Python callables can't go in `Config.tools[]`
  ...
  - Composition over special-casing. LocalTools accepts a `fallback`
    ToolDriver so users can mix their callables with a runner's
    built-ins (e.g. `ShellTools` from aep-anthropic): callables
    LocalTools knows about win, everything else falls through.
  - Verifier-aware. `pre_tool:<name>` verifiers fire on local-tool
    dispatch the same as RPC tools — the runner gates BEFORE calling
    `is_local`, so approval gates / shell verifiers work uniformly.
  - Schemas exported. `.schemas` returns the `{name, description,
    input_schema}` list runners hand to the model so it can call them.
    Same shape as `aep_anthropic.shell_tools.SHELL_TOOL_SCHEMAS`.
...
class LocalTools(ToolDriver):
    ...
    user-defined tools over runner built-ins:

        tools = LocalTools(fallback=ShellTools())

    The runner sees a single `ToolDriver`. LocalTools' callables win
    ...
    def schemas(self) -> list[dict[str, Any]]:
        ...
        runner's tool-surface builder so the model knows about these
        tools. For aep-anthropic, pass to
