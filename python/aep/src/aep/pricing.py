"""Shared pricing for AEP runners.

Why this lives in `aep` core, not per-runner:
- Both `aep-anthropic` and `aep-claude-agent` need the same model-price
  lookup. Two copies drift the moment Anthropic ships new pricing.
- The price table is data, not policy — runners load it at startup,
  users can override via the public `PriceTable` type, and the on-wire
  cost number is tagged with `aep.cost.source` so downstreams can tell
  a locally-computed number from a provider-reported one (or unknown).

Default ships in `aep/data/prices.json`; `load_default_prices()` reads it
...
The runner stamps `aep.cost.source` on `model_turn_ended` and
...
def load_default_prices() -> dict[str, ModelPrice]:
    """Load the bundled default price table from `aep/data/prices.json`.
    ...
    """
    raw = resources.files("aep.data").joinpath("prices.json").read_text()
...
def compute_cost(
    ...
    *,
    ...
    Returns `(cost, source)` so callers can stamp `aep.cost.source` on
    the wire alongside `aep.cost_usd`.

    `input_tokens` here is the AEP convention (cache reads INCLUDED).
