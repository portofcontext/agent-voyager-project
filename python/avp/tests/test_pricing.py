...
def test_model_turn_ended_emits_computed_when_driver_default() -> None:
    ...
    agent = AVPAgent(
        config=Commission(schema_version="0.1", run_id="cost-default", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=10,
...
def test_model_turn_ended_emits_reported_when_driver_says_so() -> None:
    ...
    agent = AVPAgent(
        config=Commission(schema_version="0.1", run_id="cost-prov", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=10,
...
def test_model_turn_ended_serializes_cost_source_under_dotted_alias() -> None:
    ...
    agent = AVPAgent(
        config=Commission(schema_version="0.1", run_id="cost-wire", model="test/mock"),
        model=ScriptedModel(
            [
                ModelResponse(
                    tokens_input=1,
