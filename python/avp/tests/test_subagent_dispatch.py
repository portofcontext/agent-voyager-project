"""Seam test for subagent dispatch through AVPAgent.

Crosses agent ↔ subagent_driver: the parent's scripted model emits a
tool_use whose name matches a declared Subagent; the agent's
...
def test_subagent_call_emits_invoked_and_returned_with_paired_span_id() -> None:
    ...
    assert invoked[0].data.avp_subagent_input == {"prompt": "go"}
    assert returned[0].data.avp_subagent_result_text == "all clear"
    assert returned[0].data.avp_subagent_reason == "converged"
...
def test_subagent_with_no_driver_emits_subagent_failed() -> None:
    ...
    assert failures[0].data.avp_subagent_error_code == "not_configured"
    # And no subagent_returned was emitted.
    assert not _by_type(agent.trajectory, SubagentReturnedEvent)
...
def test_subagent_collision_with_tool_name_aborts_run() -> None:
    ...
    agent.run()
    errors = _by_type(agent.trajectory, ErrorOccurredEvent)
    stops = _by_type(agent.trajectory, AgentStoppedEvent)
    assert len(errors) == 1 and "collide" in errors[0].data.avp_error_message
    assert len(stops) == 1 and stops[0].data.avp_reason == "error"
...
def test_subagent_appears_in_agent_started_subagents_field() -> None:
    ...
    `agent_started.data.subagents`. Consumers don't need to parse Commission
    ...
    agent.run()
    started = _by_type(agent.trajectory, AgentStartedEvent)[0]
...
def test_allowed_tools_filters_subagent_names() -> None:
    ...
    errors = _by_type(agent.trajectory, ErrorOccurredEvent)
    assert len(errors) == 1
    assert "excluded" in errors[0].data.avp_error_message
