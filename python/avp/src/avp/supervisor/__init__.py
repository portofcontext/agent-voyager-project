...
v0.1 has one SupervisorMessage type: avp.tool_exec_resolved (the JSON-RPC reply
...
def resolve_tool(
    ...
    *,
    ...
    """Build an avp.tool_exec_resolved event from a request.
    ...
    `server_id` (optional) flips the `source` URI to `avp://mcp/<server_id>`
    ...
    a placeholder. The agent overrides it with the rpc-span context when it
    ...
    data = ToolExecResolvedData(
        trace_id=new_trace_id(),  # placeholder; agent restamps on receipt
        span_id=new_span_id(),
        parent_span_id=ZERO_SPAN_ID,
        rpc=rpc,
        **{"avp.request_id": request.data.avp_request_id},
    )
