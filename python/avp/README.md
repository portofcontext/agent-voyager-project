...
- **Wire types** (`avp.types`) — Pydantic v2 models for every Commission, Event, and AgentDescriptor variant in v0.1, with discriminated unions on `type`. Also surfaced as sub-spec-scoped re-exports under `avp.trajectory`, `avp.commission`, `avp.descriptor`, and `avp.resolver` for consumers that want a narrow import.
