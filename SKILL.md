...
3. The agent emits the full lifecycle: `run_requested`, `agent_described`, `agent_started` (carrying the merged `tools[]` with MCP-surfaced entries tagged `avp.mcp_server_id`, and `mcp_servers[]` with each attempted dial's `status`), `managed_ref_resolved` (per asset), `skill_loaded` (per resolved skill), `model_turn_started/ended`, `tool_invoked/returned`, `agent_stopped`. Every event is a CloudEvents 1.0 envelope. Per-turn cost / token deltas ride on `model_turn_ended`; the supervisor reduces them to compute totals (spec §7.1).
